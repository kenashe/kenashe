...
This is a PILLAR: the definitive current-state reference on this topic for The Lab, not a reaction to one day's news. Map where the topic actually stands: what is settled, what is contested, what a practitioner should do right now, and what to watch next. Answer the sub-questions a serious reader (or an AI assistant answering them) would expect, each as its own H2. Write the piece you would send someone who says "catch me up on this," and add a forward-looking judgment they could not get from any single source. You are usually given several related primary sources from this area; build the throughline across them, what they agree on, where they diverge, and what they collectively imply, rather than leaning on any single one.

Sourcing bar (a pillar carries authority, so hold it higher than a daily note): anchor the piece on primary sources you can name precisely, the paper, the official post, the disclosure, with its title and author or lab, and prefer them over second-hand commentator recaps. Any specific model name, product, incident, or number you cannot attribute to a named source in the material below must be cut or explicitly marked as unconfirmed (name who is claiming it and note that it is unverified). Never present a list of specifics you cannot source. If the strongest available sourcing for a consequential claim is commentary, say so plainly and treat the claim as reported, not confirmed. A narrower, fully-sourced pillar beats a comprehensive one built on rumor. When you build on a paper or an official post, cite it precisely: its exact title and author names, plus its identifier (for a paper, the arXiv ID) whenever the provided material includes one, so every claim is verifiable. Copy those details verbatim from the sources below, and never invent a missing author list or ID. Attribute each specific statistic to the source it came from, rather than stating numbers free-floating.${spokes.length ? `
